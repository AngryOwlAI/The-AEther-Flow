...
Use this contract for Mermaid diagrams produced with the
`mermaid-documentation` subskill in The Aether-Flow repository. The values are
stored as literal hex colors because Mermaid theme variables are not CSS
variable aware.
...
For governed registered Markdown in this repository, prefer renderer defaults
and explicit `classDef` classes. Do not add Mermaid YAML frontmatter to
governed registered diagrams unless the validator has been extended to accept
it.

## Palette Boundary

Use only this local palette for fills, strokes, text, edge lines, and edge
labels unless a task explicitly replaces the palette:
...
New colors require an explicit palette update outside the diagram being edited.

## Theme Variables

The renderer default config should use this baseline. Non-governed scratch
diagrams may use it as Mermaid frontmatter when the target renderer accepts
frontmatter:
...
Use these semantic classes as the default starter library:
...
- `external`: external systems or provenance links outside the primary journey.
