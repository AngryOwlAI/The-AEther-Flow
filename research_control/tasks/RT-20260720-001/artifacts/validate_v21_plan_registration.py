#!/usr/bin/env python3
"""Validate the bounded v21 P0-T01 plan-registration transaction."""
...
TASK_DIR = ROOT / "research_control/tasks/RT-20260720-001"
...
def main() -> int:
    ...
    require(report["plan_sha256"] == sha256(plan_bytes), "V21-REG-018", "registration report SHA-256 mismatch", findings)
    require(report["plan_git_blob_sha1"] == git_blob_sha1(plan_bytes), "V21-REG-019", "registration report Git blob mismatch", findings)
    require(report["observed_counts"]["phase_count"] == len(phases), "V21-REG-020", "reported phase count mismatch", findings)
    require(report["observed_counts"]["task_card_count"] == len(task_cards), "V21-REG-021", "reported work-item count mismatch", findings)
    require(report["recommendation_inventory"] == EXPECTED_RECOMMENDATIONS, "V21-REG-022", "reported inventory mismatch", findings)
    require(report["coverage_matrix_recommendations"] == EXPECTED_RECOMMENDATIONS, "V21-REG-023", "reported coverage mismatch", findings)
    require(all(value is False for value in report["non_authority_checks"].values()), "V21-REG-024", "non-authority report contains an authorized protected action", findings)

    require('active_task_id: "RT-20260720-001"' in program_state, "V21-REG-025", "program state active task mismatch", findings)
    require('latest_handoff_id: "handoff-0773"' in program_state, "V21-REG-026", "program state control handoff mismatch", findings)
    require('latest_research_handoff_id: "handoff-0772"' in program_state, "V21-REG-027", "scientific handoff preservation missing", findings)
    require('route_id: "v21_p0_t02_backlog_materialization"' in program_state, "V21-REG-028", "P0-T02 route missing", findings)

    for source in hash_receipt["sources"]:
        path = ROOT / source["path"]
        require(path.is_file(), "V21-REG-029", f"hash source missing: {source['path']}", findings)
        if path.is_file():
            data = path.read_bytes()
            require(source["current_sha256"] == sha256(data), "V21-REG-030", f"SHA-256 mismatch: {source['path']}", findings)
            require(source["current_git_blob_sha1"] == git_blob_sha1(data), "V21-REG-031", f"Git blob mismatch: {source['path']}", findings)
