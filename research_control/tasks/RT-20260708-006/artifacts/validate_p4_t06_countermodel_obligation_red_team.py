#!/usr/bin/env python3
"""Validate v18 P4-T06 countermodel-obligation red-team outputs."""
...
JOB_ID = "AJ-RT-20260708-006-001"
ARTIFACT = (
    ROOT
    / "research_control/tasks/RT-20260708-006/artifacts/"
    / "countermodel_obligation_red_team_review_v1.md"
)
REPORT = (
    ROOT
    / "research_control/tasks/RT-20260708-006/artifacts/"
    / "p4_t06_countermodel_obligation_red_team_report.json"
)
ALLOWED_RESULTS = {"pass", "repair_required", "fail_closed"}
REQUIRED_DONE_CRITERIA = {
    "review_result_allowed",
    "obligations_not_theorem_substitute",
    "pass_routes_to_p5_t01",
}
...
def validate() -> dict[str, Any]:
    ...
    if data.get("p4_review_result") not in ALLOWED_RESULTS:
        failures.append("p4_review_result must be pass repair_required or fail_closed")
    if data.get("p4_review_result") != "pass":
        failures.append("P4-T06 expected pass for this packet")
    ...
    if data.get("recommended_next_route") != "P5-T01":
        failures.append("recommended_next_route must be P5-T01 on pass")
    ...
    allowed_claims = " ".join(str(item) for item in _as_list(claim_boundary, "allowed_claims"))
    combined_claim_text = f"{allowed_claims} {forbidden_claims} {data.get('claim_under_review', '')}"
    ...
    if not pressure_points:
        failures.append("external_mathematical_pressure_points must be nonempty")
    ...
    return {
        "schema_id": "p4_t06_countermodel_obligation_red_team_validator_v1",
        "status": "PASS" if not failures else "FAIL",
        "failures": failures,
        "artifact_path": ARTIFACT.relative_to(ROOT).as_posix(),
        "global_schema_summary": schema_summary,
        "review_result": data.get("p4_review_result"),
