...
purpose: "Explain how AEther-Flow active roles, superseded role versions, human-gated roles, skill entry points, and descriptive role-perspective definitions should be navigated without treating the catalog as execution authority."
...
These definitions are descriptive guidance only. They do not create role
...
Roles And Skills Catalog function, what perspectives selected workflow
entries provide, which source paths ground it, which adjacent systems
...
main reader sections are `Perspective Definitions`, `Authority Inspection Order`, `Active Role Families`, `Skill Workflow Map`, `Catalog Overreads`. The HTML derivative may render these
...
Reader scope: role and skill navigation only. This page cannot change role
status, register roles, supersede roles, expand role authority, change skill
contracts, change validator behavior, change routing, change allowlists, or
...
- States that both perspective definitions are descriptive guidance only and do not change role authority, role semantics, validators, routing, permissions, checkpoint behavior, generated-output authority, or physics claim authority.
