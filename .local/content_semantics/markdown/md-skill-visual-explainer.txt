...
metadata:
  author: nicobailon
  version: "0.7.1-project-aether-flow"
---
...
Detailed prompt templates in `./commands/`. In Pi, these are slash commands (`/diff-review`). In Claude Code, namespaced (`/visual-explainer:diff-review`). In Codex, use `/prompts:diff-review` when prompt templates are available. This project also vendors selected prompt templates under `.codex/prompts/`, or you can invoke `$visual-explainer` and describe the workflow.
...
| `aether-flow-project-explainer` | Project-specific explainer for the Æther-Flow research program |

This project-local installation intentionally does not include the upstream
`fact-check` prompt because that workflow edits documents in place. It also
does not include the upstream `share-page` prompt or `share.sh` script because
generated research pages may contain unpublished, speculative, or noncanonical
material. Public deployment requires a separate explicit user request and a
fresh review of the page contents.

## Æther-Flow Project Mode

Use this mode when the user asks to explain this repository, the research
program, the ontology, the exact-GR benchmark, the manuscript set, or the
...
**Claim boundaries:**
- Label the physics track as exact-GR benchmark adoption plus an open
  derivation burden.
- Label the AI track as a human-scaffolded research-agent workflow, not an
...
- Treat generated visual pages as explanatory artifacts, not source authority.

**Output path for project explainers:** write to
`.local/html_wikis/visual-explainer/` with a descriptive filename. Open the
...
For governed Documentation Curator publication work, use
`research_control/design/documentation_curator_publication_process.md`,
`registries/PUBLICATION_BRIEF_REGISTRY.csv`, and the relevant publication
brief under `markdown/publication-briefs/`. Design from the brief and source
bundle; do not fall back to a universal page skeleton. Tracked public docs must
not use NPX, `@agent-native/core`, hosted Plan MCP, localhost bridge tooling,
CDN diagram rendering, remote CSS, remote fonts, external analytics, hosted
comments, or browser-side Mermaid execution.
...
Tracked project explainers must implement the publication brief:

- open with subject framing, not source metadata;
- expose visible source grounding and a clear noncanonical boundary;
- choose headings and visual treatment for the page's reader job;
- let GitHub Markdown and HTML diverge where the media differ;
- avoid repeated section skeletons across unrelated topics; and
- preserve source and claim parity without requiring visible section parity.

Use concise local enhancements only when the page still reads without
JavaScript. Navigation, source chips, and copy buttons are acceptable when they
support the brief. Do not add generic global modes or controls unless the
brief requires them and browser QA verifies them.

Run `scripts/validate_publication_process.py --root .` after governed
publication work. Use screenshot evidence and a human-facing review artifact
for pilot pages because deterministic scripts cannot certify professional
editorial quality.

Mermaid is brief-guided, not mandatory. Process, authority, memory, and claim
boundary pages often merit a registered Mermaid overview. Reference pages may
be clearer as tables, matrices, tier cards, or catalog panels. Do not add a
full deterministic HTML generator; render creatively from the spec while
keeping source-path evidence markers intact.
...
current source specs or generated HTML. Use `data-explainer-control` and
`data-source-path` markers for validator evidence. Manually author summary
prose per source spec when the brief calls for it.
...
**Output location:** Write to `~/.agent/diagrams/`. Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. The directory persists across sessions. For Æther-Flow project explainers, use `.local/html_wikis/visual-explainer/` instead.
...
## Sharing Disabled In This Project

This project-local installation omits the upstream public sharing workflow.
Generated pages may include unpublished research state, speculative physics
interpretation, or noncanonical explanatory synthesis. Do not deploy visual
explainer output to a public URL unless the user explicitly requests public
sharing after reviewing the local HTML page.
