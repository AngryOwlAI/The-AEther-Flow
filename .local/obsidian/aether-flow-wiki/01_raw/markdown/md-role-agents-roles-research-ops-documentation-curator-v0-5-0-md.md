...
# Documentation Curator v0.5.0

Superseded by `documentation-curator@0.6.0` for future reader-layer cleanup
and visible metadata boundaries. This version is retained for historical
execution records from the HTML explainer depth upgrade kit.
...
source-backed summary blocks, documentation-depth review, and
...
ground it, what the page may legitimately claim, what it may not claim, and
...
or project-control pages, include an explicit authority-status or claim-boundary
sentence. Short source quotation panels are allowed when exact source wording
clarifies a boundary, but they must remain source-backed and must not promote
Markdown prose above registered scientific or registry authority.
...
- Must render non-summary content blocks as completed documentation rather than
  source-spec instructions.
