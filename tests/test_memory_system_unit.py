...
FIXTURE_ROOT = REPO_ROOT / "tests" / "fixtures" / "memory_system" / "mini_repo"
...
def fixture_registry_rows(memory_system, names: list[str]) -> dict[str, list[dict[str, str]]]:
    return {
        name: memory_system.read_csv_rows(FIXTURE_ROOT / "registries" / name)
        for name in names
    }


def fixture_project_directories() -> list[str]:
    directories = {"."}
    directories.update(
...
def valid_synthetic_html_text(extra_body: str = "") -> str:
    return (
        ...
        "<style>"
        "p, td, th, .publication-card { overflow-wrap: break-word; }"
        ...
        "</style>"
        "<main><p>Source-backed fixture.</p></main>"
        f"{extra_body}"
...
class MemorySystemUnitTests(unittest.TestCase):
    @classmethod
    ...
    def test_fixture_repository_registries_validate(self) -> None:
        rows_by_registry = fixture_registry_rows(
            self.memory_system,
            self.memory_system.SOURCE_REGISTRY_NAMES
            + self.memory_system.GENERATED_REGISTRY_NAMES,
        )
        report = self.memory_system.ValidationReport()
        with mock.patch.object(self.memory_system, "REPO_ROOT", FIXTURE_ROOT):
            self.memory_system.validate_pdf_registry(report, rows_by_registry)
            self.memory_system.validate_wiki_registry(report, rows_by_registry)
            self.memory_system.validate_file_object_registry(report, rows_by_registry)
    ...
    def test_active_documentation_curator_role_uses_publication_validator_only(self) -> None:
        rows = self.memory_system.read_csv_rows(
            FIXTURE_ROOT / "registries" / "AGENT_ROLE_REGISTRY.csv"
        )
        active_curators = [
            row
            for row in rows
    ...
    def test_stale_pdf_source_hash_is_an_error(self) -> None:
        rows_by_registry = fixture_registry_rows(
            self.memory_system, self.memory_system.SOURCE_REGISTRY_NAMES
        )
    ...
    def test_html_spec_contract_allows_page_specific_controls_to_be_omitted(self) -> None:
        report = self.memory_system.ValidationReport()
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir).resolve()
        ...
        self.assertFalse(any("missing required_controls" in error for error in report.errors))
    ...
    def test_html_registry_rejects_obsolete_summary_label(self) -> None:
        report = self.memory_system.ValidationReport()
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir).resolve()
            spec = root / "markdown/html-explainer-specs/synthetic.md"
            ...
            html.write_text(
                valid_synthetic_html_text(
                    extra_body="<h2>What This Explainer Describes</h2>"
                ),
                encoding="utf-8",
            )
            rows_by_registry = {
        ...
        self.assertTrue(
            any(
                "obsolete visible summary label is present" in error
                for error in report.errors
            )
        )
    ...
    def test_html_registry_rejects_obsolete_analysis_capsule_layer(self) -> None:
        ...
        self.assertTrue(
            any("retired claim boilerplate must not render" in error for error in report.errors)
        )
    ...
    def test_html_registry_rejects_unreadable_three_layer_layout(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            html.write_text(
                ...
                '<meta name="aether-flow-human-visual-only" content="true">'
                "<style>"
                "p, td, th, .publication-card { overflow-wrap: anywhere; }"
    ...
    def test_folder_map_mentions_task_artifact_relationships(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                self.memory_system,
                "project_directories",
                return_value=fixture_project_directories(),
            ),
        ):
