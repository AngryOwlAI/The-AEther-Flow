...
`.local/` is repo-local retrieval/cache state. It may be regenerated by
bootstrap or sync commands, but it must remain untracked and must not be used
as committed transaction evidence or scientific authority.
...
`.local/` caches never override tracked control state. Research-control
AgentJobs must not list `.local/**` as write output unless a bounded
project-system cleanup packet is explicitly removing tracked cache state from
the Git index or repairing local retrieval tooling.
...
- Keep `.local/` for scratch builds, caches, previews, and experiments. It is
  ignored repository-local state, not a tracked generated-output surface.
