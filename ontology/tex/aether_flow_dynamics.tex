...
The foundations manuscript fixes the selected source architecture of
\TheoryProgram{}: a smooth four-dimensional source manifold is explicit
primitive debt, and \(\PhiSrc\) is an unresolved abstract source-order or
evolution slot. It does not identify the source arena with physical spacetime,
type \(\PhiSrc\), or adopt a source-to-target bridge. The present paper states
the separate target-side dynamical content of \TheoryName{}.

\paragraph{Framework and claim boundary.}
\TheoryProgram{} denotes the broader \Aether{} / \Aflow{} research
interpretation. Its canonical source package presently contains only the
continuum-first source arena and unresolved \(\PhiSrc\) slot described above.
The familiar experiential-slice, S-time, and deeper-motion language remains
interpretive motivation and open reconstruction work, not source dynamics.
Within that framework, \TheoryName{} denotes the exact-closure benchmark in
which the effective gravitational dynamics are adopted to be exactly
Einsteinian with universal matter coupling. In the active sequence,
\emph{adoption} means use of that established target-side dynamics without
claiming source derivation, while \emph{derivation} is reserved for a
first-principles recovery from explicit source variables.
...
The operative consequence is straightforward. In \TheoryName{}, the
gravitational dynamics are taken to be Einsteinian exactly. The selected
source architecture and \Aether{} / \Aflow{} interpretation do not add an
extra low-energy propagating sector, and no source dynamics is inferred from
...
\TheoryProgram{} in which the selected source research architecture remains
distinct from the effective gravitational dynamics, which are exactly those
of general relativity with universal matter coupling.
\end{definition}
...
In particular, \TheoryName{} does not introduce a second established matter
metric, a direct preferred-frame matter coupling, or a low-energy matter
interaction proportional to an independently measurable \Aflow{} correction.
The source architecture and interpretation remain separate, while the
effective matter law is the ordinary GR law.
...
\begin{remark}
Within the broader \TheoryProgram{}, one may investigate whether this
effective metric can be reconstructed from source data. No such bridge is
adopted here: the source manifold is not the physical spacetime by fiat, and
\(\PhiSrc\) is not the metric, a congruence, or physical time. This open
interpretive aim does not alter the exact effective equations used in
\TheoryName{}.
\end{remark}
...
In \TheoryName{}, the \Aflow{} does not appear as an additional low-energy
propagating degree of freedom in the effective action \eqref{eq:SA}. At source
level, \(\PhiSrc\) remains unresolved; at target level, flow language may be
used only as an interpretive dictionary inside the adopted GR geometry.

This means:
\begin{itemize}
    \item \(\PhiSrc\) is a placeholder for later source-order or evolution semantics, not an adopted map, action, law, or generator;
    \item S-time remains an open reconstruction target rather than an adopted clock or monotonicity law;
    \item foliation or congruence language belongs to the target-side interpretive dictionary unless a later source-to-target bridge is derived;
...
What remains open is the derivational program. In schematic form, a future
source manuscript would have to specify explicit variables \(Q\) on the
selected source arena and an action of the form
\begin{equation}
S_{\Sub}
=
...
\end{equation}
together with a definite map from those variables to the effective
relativistic sector. No source metric, order field, action, variation law, or
induced Lorentzian metric is part of the selected primitive package. Those
objects remain future constructions rather than adopted dynamics.
...
This paper makes one clean choice: \TheoryName{} is formulated here as the
exact-closure dynamics benchmark of \TheoryProgram{}. The source arena and
\(\PhiSrc\) slot do not supply these dynamics. The effective content is exact
and explicit: Einstein--Hilbert plus ordinary matter, with the ordinary
cosmological-constant completion or equivalent GR-compatible dark-energy
sector available inside the same one-metric law. Its field equations are the
Einstein equations. Its matter law is universal minimal coupling to a single
effective metric. Its null, timelike, and freely falling structures are
exactly relativistic. Its weak-field, Newtonian, and local inertial limits are
exactly those of GR.
