...
The central move is to provide a target-side interpretive dictionary for
\Aflow{} using a future-directed unit timelike congruence \(u^\mu\) on the
exact relativistic spacetime and the standard decomposition of
\(\nabla_\mu u_\nu\) into expansion, shear, vorticity, and acceleration. The
congruence is an admissible, generally non-unique observer field in the
adopted GR benchmark. It is not the unresolved source slot \(\PhiSrc\), and no
source-to-congruence bridge is claimed. Expansion describes local volume
change and cosmological growth of the observer congruence. Acceleration
governs static gravitational support and redshift structure. Vorticity
captures swirl, rotation, and frame dragging. Shear captures anisotropic
distortion, tidal structure, and the strain content associated with
gravitational waves. Local special relativity is recovered as the local
inertial limit of the same target-side geometry.

The resulting claim is precise. In \TheoryName{}, \Aflow{} is not a second
low-energy gravitational field competing with the metric. Congruence
kinematics provides a disciplined target-side reading of the historical flow
vocabulary, while the canonical source architecture remains a smooth
four-dimensional arena carried as primitive debt plus unresolved
\(\PhiSrc\). This interpretation neither identifies the source arena with
physical spacetime nor pretends that a first-principles bridge has been
completed.
...
The active \TheoryName{} sequence fixes three separate points. First, the
canonical mainline research architecture uses a smooth four-dimensional
source manifold as explicit primitive debt and retains \(\PhiSrc\) only as an
unresolved abstract source-order or evolution slot. Second, the exact
effective gravitational law of the benchmark is general relativity with
universal matter coupling. Third, no map between those source and target
objects is adopted. The task of this manuscript is therefore to discipline the
target-side flow vocabulary without collapsing it into source structure.
...
\TheoryProgram{} denotes the broader \Aether{} / \Aflow{} research
interpretation. Its source package does not adopt a physical metric, causal
cone, clock, slice embedding, matter sector, dynamics, or physical meaning for
\(\PhiSrc\). The familiar experiential-space, S-time, and deeper-motion
phrases remain interpretive aims. Within that framework, \TheoryName{} denotes
the exact-closure benchmark in which the effective gravitational dynamics are
adopted to be exactly Einsteinian with universal matter coupling. In the
active sequence, \emph{adoption} means use of that established target-side
dynamics without claiming source derivation, while \emph{derivation} is
reserved for a first-principles recovery from explicit source variables.
...
\label{eq:unitnorm}
\end{equation}
The congruence provides the observer-based target-side kinematic dictionary by
which the historical \Aflow{} vocabulary is read inside the exact relativistic
geometry. This definition does not identify \(u^\mu\) with \(\PhiSrc\) or
construct either object from the other.
\end{definition}

\begin{remark}
The congruence is not a second field equation added to GR. It is an admissible,
generally non-unique observer field defined on the exact metric spacetime.
Different physical situations may call for different natural congruences, but
they all live inside the same target metric geometry. No source-level
derivation of a preferred choice and no bridge from \(\PhiSrc\) is claimed
here.
\end{remark}
...
The observer field \(u^\mu\) is an admissible congruence choice inside the
adopted target metric geometry, not a unique field derived from source
dynamics. At current scope there is no source-to-congruence map: one chooses
the observer family that fits the target-side physical question while
remaining inside the same exact relativistic spacetime
\cite{Raychaudhuri1955,EllisVanElst1999}.
...
\section{Target-Side Kinematic Flow Dictionary}
...
\begin{remark}
This clarifies only the target-side dictionary. Congruence expansion inside
the exact relativistic geometry is not evidence that \(\PhiSrc\) has an
adopted evolution law or that observed expansion has been derived from source
structure.
\end{remark}
...
This is the correct exact-closure location of dark energy in the target-side
\Aflow{} dictionary. Dark energy is not a literal fluid wind pushing galaxies
through a pre-existing space. It is the standard GR cosmological term or
stress tensor that changes the evolution of the same congruence expansion
\(\theta=3H\). No inference from this target-side acceleration to source
\(\PhiSrc\), a source law, or a metric derivation is licensed.
...
\begin{enumerate}
    \item It gives the historical \Aflow{} vocabulary a congruence-based target-side interpretation without identifying the congruence with \(\PhiSrc\), a second low-energy gravitational field, or a source dynamics.
...
\begin{enumerate}
    ...
    \item It does not claim a first-principles source derivation of the congruence dictionary or of Einsteinian gravity from explicit source variables.
    \item It does not type \(\PhiSrc\), identify the source arena with physical spacetime, or supply a source-to-congruence bridge.
...
The clean way to make the \Aether{} and \Aflow{} interpretation useful inside
\TheoryName{} is not to oppose GR or SR or to identify source and target
objects by fiat. It is to read the exact relativistic geometry through a
disciplined target-side dictionary while keeping the source bridge open. The
congruence-based formalism provides that dictionary.

In this target-side language, the historical flow idea is distributed across
structured relativistic quantities. Expansion \(\theta\) captures ordered
growth and cosmological volume change. Acceleration \(a_\mu\) captures static
gravitational support and redshift structure. Vorticity
\(\omega_{\mu\nu}\) captures swirl, rotation, and frame dragging. Shear
\(\sigma_{\mu\nu}\) captures anisotropic tidal distortion and wave strain.
Local special relativity appears as the local inertial limit of the same
geometry. None of these target-side quantities supplies the missing semantics
or dynamics of \(\PhiSrc\).

That is the appropriate status of the manuscript. It does not claim that GR
has been derived from source microphysics or that the selected source arena is
physical spacetime. It claims only that, once exact closure is adopted, the
\Aether{} / \Aflow{} interpretation has a disciplined target-side flow
dictionary. A first-principles connection from \((\Sub,\PhiSrc)\) to that
measured relativistic world remains open.
